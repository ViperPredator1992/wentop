...
class ModelExtensionModuleProgromanCityManager extends Model {
    ...
    public function install() {
        ...
        if (empty($this->db->query("SELECT COUNT(*) total FROM prmn_cm_city")->row['total'])) {
            $this->db->query("
                INSERT INTO `prmn_cm_city` (`fias_id`, `name`)
                 SELECT `fias_id`, `offname` 
                 FROM `fias` 
                 WHERE `fias_id` IN (41,86,5033,3734,3145,3187,4006,3753,2990,5147,3704,4131,4187,2782,3737,6125,2638,4778)
                 ORDER BY IF (`fias_id` IN (41,86), 0, 1), `offname`;
             ");
        }
