...
class ActionDownloadCountry extends DatabaseFileAction {
    ...
    private function stepUpload() {
        ...
        try {
            $this->uploadFile('fias', $upload_dir . '/' . $this->country['iso'] . '.zip');
        } catch (\Exception $e) {
    ...
    protected function makeParamsForProgromanServer() {
        return parent::makeParamsForProgromanServer() . '&country=' . $this->country['iso'] . '&db_version=' . $this->country['version'];
    }
